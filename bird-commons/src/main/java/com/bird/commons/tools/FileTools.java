...
public class FileTools {
    ...
    public static String readFile(InputStream inputStream, Charset charset) throws IOException {
        if (inputStream == null) {
            ...
            while ((tmp = reader.readLine()) != null) {
                builder.append(tmp).append(Const.NEW_LINE);
            }
    ...
    public static String readFile(InputStream inputStream) throws IOException {
        return readFile(inputStream, Const.CHARSET_UTF8);
    }
    ...
    public static void writeFile(File target, String data, boolean append) throws IOException {
        ...
        OutputStreamWriter writer = new OutputStreamWriter(stream, Const.CHARSET_UTF8);
        writer.write(data);
        writer.flush();
        writer.close();
        stream.close();
    ...
    public static void writeFile(String file, String data) throws IOException {
        writeFile(new File(file), data, Boolean.FALSE);
    }
    ...
    public static void zeroCopy(File source, File target) throws IOException {
        ...
        input.transferTo(Const.ZERO, input.size(), output);
        input.close();
        output.close();
    }
}
