...
import com.bird.demo.infrastructure.bpmn.json.util.CollectionUtils;
import com.bird.demo.infrastructure.bpmn.json.util.JsonConverterUtil;
...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    ...
    public ObjectNode convertToJson(BpmnModel model) {
        ...
        modelNode.set(EDITOR_BOUNDS, BpmnJsonConverterUtil.createBoundsNode(maxX, maxY, 0, 0));
        modelNode.put(EDITOR_SHAPE_ID, model.getMainProcess().getId());

        ObjectNode stencilNode = objectMapper.createObjectNode();
        stencilNode.put(EDITOR_STENCIL_ID, "BPMNDiagram");
        modelNode.set(EDITOR_STENCIL, stencilNode);
        ...
        if (model.getPools().size() > 0 && poolHasDI) {
            for (Pool pool : model.getPools()) {
                ...
                poolNode.set(EDITOR_OUTGOING, outgoingArrayNode);
                ...
                if (process != null) {
                    ...
                    for (Lane lane : process.getLanes()) {
                        ...
                        laneNode.set(EDITOR_OUTGOING, objectMapper.createArrayNode());
        ...
        if (objectNode.get(EDITOR_CHILD_SHAPES) != null) {
            for (JsonNode jsonChildNode : objectNode.get(EDITOR_CHILD_SHAPES)) {
                ...
                if (!STENCIL_SEQUENCE_FLOW.equals(stencilId)) {
                    ...
                    ArrayNode outgoingNode = (ArrayNode) jsonChildNode.get(EDITOR_OUTGOING);
