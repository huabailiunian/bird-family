...
public class FlowableCollectionParser extends BaseChildElementParser {
    ...
    public void parseChildElement(XMLStreamReader xtr, BaseElement parentElement, BpmnModel model) throws Exception {
        ...
        if (StringUtils.isNotEmpty(xtr.getAttributeValue(null, BpmnXMLConstants.ATTRIBUTE_MULTIINSTANCE_COLLECTION_CLASS))) {
            collectionHandler = new CollectionHandler();
        	collectionHandler.setImplementation(xtr.getAttributeValue(null, BpmnXMLConstants.ATTRIBUTE_MULTIINSTANCE_COLLECTION_CLASS));
        	collectionHandler.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_CLASS);
        	
        } else if (StringUtils.isNotEmpty(xtr.getAttributeValue(null, BpmnXMLConstants.ATTRIBUTE_MULTIINSTANCE_COLLECTION_DELEGATEEXPRESSION))) {
            collectionHandler = new CollectionHandler();
        	collectionHandler.setImplementation(xtr.getAttributeValue(null, BpmnXMLConstants.ATTRIBUTE_MULTIINSTANCE_COLLECTION_DELEGATEEXPRESSION));
        ...
        try {
            while (!readyWithCollection && xtr.hasNext()) {
                xtr.next();
                if (xtr.isStartElement() && BpmnXMLConstants.ELEMENT_MULTIINSTANCE_COLLECTION_STRING.equalsIgnoreCase(xtr.getLocalName())) {
            		// it is a string value
                    multiInstanceLoopCharacteristics.setCollectionString(xtr.getElementText());
                    
                } else if (xtr.isStartElement() && BpmnXMLConstants.ELEMENT_MULTIINSTANCE_COLLECTION_EXPRESSION.equalsIgnoreCase(xtr.getLocalName())) {
            		// it is an expression
    ...
    public String getElementName() {
        return BpmnXMLConstants.ELEMENT_MULTIINSTANCE_COLLECTION;
    }
}
